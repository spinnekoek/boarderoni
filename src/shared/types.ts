...
export interface ToggleSwitchWidget extends SwitchWidgetBase, WidgetVisibility {
   ...
   * Width/Height/BorderColor/BorderWidth/BorderRadius below — capping the lever's own tip for
...
export interface DeckExportFile {
  ...
  exportedAt: number
  // app.getVersion() at export time. Compared on import against this
  // install's own version (see collectImportWarnings in main/index.ts) —
  // only flagged as a non-blocking warning when the export is newer, since
  // it may lean on a feature/default this install doesn't understand yet.
  // Distinct from DECK_EXPORT_FORMAT_VERSION (formatVersion above), which is
  // enforced outright: this is a newer *app version* that's otherwise still
  // a compatible *format*.
  appVersion: string
  dashboard: Dashboard
  ...
  // actually works on the machine it's imported onto. Absent (rather than
  // []) when the deck plays no sounds at all, matching backgroundImage
  ...
  // Every custom font any of this deck's labels references (WidgetLabel.
  // fontFamily), bundled the same way sounds are above — see
  // collectDeckFonts in main/index.ts. A font file's own licence may not
  // allow redistribution, so bundling isn't silent: the export flow shows a
  // one-time notice asking the exporter to confirm they're allowed to share
  // whichever fonts get bundled here (see handleExport in DeckPicker.tsx).
  // Ids are preserved on import for the same reason sounds' are.
  fonts?: { id: string; label: string; filename: string; dataBase64: string }[]
}
...
export interface RestWebhookTarget {
  ...
  // Skips TLS certificate verification for THIS target's own requests only
  // (see runCallRestAction in main/index.ts, which builds a one-off undici
  // Agent with `connect: { rejectUnauthorized: false }` and passes it as
  // fetch's own `dispatcher` option when this is set) — never the blunt,
  // process-wide `NODE_TLS_REJECT_UNAUTHORIZED=0` env var, which would
  // disable verification for every outgoing request app-wide, not just this
  // one target. Defaults to false/unset (verify normally) — for a target
  // behind a self-signed or otherwise invalid certificate, e.g. an internal
  // device's own HTTPS admin API.
