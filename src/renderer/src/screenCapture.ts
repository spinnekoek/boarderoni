...
export function screenCaptureFrameUrl(deckId: string, widgetId: string, cacheBust: number): string {
  return `http://${host()}:${SERVER_PORT}/screen-capture/frame?deck=${encodeURIComponent(deckId)}&widget=${encodeURIComponent(widgetId)}&t=${cacheBust}&${contentAuthParams()}`
}
...
export function screenCaptureStreamUrl(deckId: string, widgetId: string): string {
  return `http://${host()}:${SERVER_PORT}/screen-capture/stream?deck=${encodeURIComponent(deckId)}&widget=${encodeURIComponent(widgetId)}&${contentAuthParams()}`
}
